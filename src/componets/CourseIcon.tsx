...
const CourseIcon: React.FC<CourseIconProps> = ({ type = 'default', icon, size = 'md' }) => {
  const sizeClasses = {
    sm: 'w-8 h-8',
    md: 'w-12 h-12',
    lg: 'w-16 h-16',
  ...
  return (
    <div
      className={`${sizeClasses[size]} bg-blue-100 rounded-lg flex items-center justify-center text-blue-600`}
    >
      {getIcon()}
    </div>
  );
