    ...
    <section className={`w-full py-6 lg:py-0 mb-10 md:mb-20 ${backgroundClasses[background]}`}>
      <Container>
        <div className={`text-${centered ? 'center' : 'left'} mb-12 lg:mb-16`}>
          <h2
            className={`text-3xl md:text-4xl lg:text-5xl font-bold ${textClasses[background]} mb-4`}
          >
            {title} <span className="bg-[#FFC224] px-2 rounded-md">Courses</span>
          </h2>
          <p
            className={`text-lg md:text-xl ${subtitleClasses[background]} max-w-2xl ${centered ? 'mx-auto' : ''}`}
          >
            {subtitle}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3">
          {stats.map(stat => (
            ...
            >
              <div
                className={`inline-flex items-center justify-center w-16 h-16 rounded-full ${stat.color} mb-6 group-hover:scale-110 transition-transform duration-300`}
              >
                <stat.icon className="text-2xl text-white" />
              </div>

              <div
                className={`text-4xl md:text-5xl lg:text-6xl font-bold ${textClasses[background]} mb-2`}
              >
                {stat.value}
              </div>
